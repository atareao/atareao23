...
add_filter( 'jetpack_google_fonts_list', 'atareao23_filter_jetpack_google_fonts_list' );

$atareao23_enqueued_font_slugs = array();
...
/**
 * Build a list of all font slugs provided by atareao23 from theme.json
 *
 * @return array Collection of all font slugs defined in the theme.json file
 */
function collect_fonts_from_atareao23() {
    $fonts                  = array();
    ...
    foreach ( $font_families as $font ) {
        ...
        if ( array_key_exists( 'provider', $font ) && 'atareao23-fonts' === $font['provider'] ) {
            $fonts[] = $font;
        }
    }
...
function enqueue_global_styles_fonts() {
    ...
    global $atareao23_enqueued_font_slugs;

    $font_slugs = array();
    $font_css   = '';

    if ( is_admin() ) {
        $font_families = collect_fonts_from_atareao23();
    ...
    $atareao23_enqueued_font_slugs = $font_slugs;
    ...
    wp_register_style( 'atareao23_font_faces', '' );
    wp_enqueue_style( 'atareao23_font_faces' );

    // Add the styles to the stylesheet.
    wp_add_inline_style( 'atareao23_font_faces', $font_css );
}

/**
 * Enqueue all of the fonts provided by atareao23 for FSE use
 */
function enqueue_fse_font_styles( $fonts ) {
    $fonts    = collect_fonts_from_atareao23();
    $font_css = '';
...
function enqueue_block_fonts( $content, $parsed_block ) {
    ...
    if ( ! is_admin() && isset( $parsed_block['attrs']['fontFamily'] ) ) {
        ...
        if ( ! in_array( $font_slug, $atareao23_enqueued_font_slugs, true ) ) {
            $font_css = get_style_css( $font_slug );
            if ( $font_css ) {
                $atareao23_enqueued_font_slugs[] = $font_slug;
                wp_add_inline_style( 'atareao23_font_faces', $font_css );
            }
        }
    }
 ...
 * This filters out all of the fonts atareao23 has already registered.
 */
function atareao23_filter_jetpack_google_fonts_list( $jetpack_fonts ) {
    $theme_fonts         = collect_fonts_from_atareao23();
